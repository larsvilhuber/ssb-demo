...
```shell
cd environment && docker build . --tag 69da7c49-f71b-4d08-918e-6cdccd8cd4c2; cd ..
```
...
docker run --rm \
  --workdir /code \
  --volume "$PWD/stata.lic":/usr/local/stata/stata.lic \
  --volume "$PWD/data":/data \
  --volume "$PWD/code":/code \
  --volume "$PWD/results":/results \
  69da7c49-f71b-4d08-918e-6cdccd8cd4c2 ./run
```
